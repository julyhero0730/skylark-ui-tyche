...
import { gridProps } from 'src/pages/lon/pmg/pdr/lonPmgPdrGridProps.vue'
...
const previousYear = new Date(session.sysPreviousBusinessDay).getFullYear()

const previousYearStartDate = dayjs(new Date(previousYear, 0, 1)).format('YYYY-MM-DD')
...
const topSearchPaneObject = ref([
  ...
  {
    type: 'label',
    label: i18n.global.t('label.view.currentStatus')
  },
  {
    type: 'dropdown',
    model: 'pastdueStatus',
    dropdownList: ['All', 'Past Due', 'Recovered'],
    css: 'width: 112px;'
  },
  {
    type: 'label',
    label: i18n.global.t('label.view.pastDueDate')
  },
  {
    type: 'calendarYmd',
    model: 'asFromDate',
...
const clear = () => {
  ...
  searchParam.value.pastdueStatus.value = 'All'
  searchParam.value.asFromDate.value = previousYearStartDate
  searchParam.value.asToDate.value = dayjs(session.sysPreviousBusinessDay).format('YYYY-MM-DD')
  rowCounts.value = 0
  rowData.value = []
...
const search = () => {
  searchParam.value.maxValue.value = '0'
  const newSearchParam = commonUtil.inputValueToJson('bsLonPmgPdrService', 'LONPMGPDR002', searchParam, rowData, rowCounts)
...
const nextSearch = () => {
  gridPropsUtil.nextSearchRow(gridProps.gridApi.value, 'bsLonPmgPdrService', 'LONPMGPDR002', searchParam, rowCounts)
}

const allSearch = () => {
  gridPropsUtil.allSearchRow(gridProps.gridApi.value, 'bsLonPmgPdrService', 'LONPMGPDR002', searchParam, rowCounts)
}

    ...
    return [
      {
        ...
        valueGetter: (params) => {
          if (params.data.id === 'total') {
            return 'Total' // 합계 행인 경우 빈 문자열 반환
          } else {
      ...
      },
      {
        ...
        valueGetter: (params) => {
          if (params.data.id === 'total' || params.value === 'TOTAL') {
            return params.data.rTotalAmt // 합계 행에서는 빈 문자열을 반환합니다.
          } else {
      ...
      },
      {
        ...
        valueSetter: params => {
          ...
          if (oldValue !== newValue) {
            ...
            // const rScheduleDate = dayjs(params.data.scheduleDate).format('YYYY-MM-DD')
            ...
            //   diffDays = dayjs(scheduleDate).diff(dayjs(disbursementDate), 'day')
            // } else { // 두번째 행부터는 이전 행의 scheduleDate와의 차이 계산
            //   const prevScheduleDate = dayjs(params.api.getDisplayedRowAtIndex(params.node.rowIndex - 1).data.scheduleDate).format('YYYY-MM-DD')
            //   diffDays = dayjs(scheduleDate).diff(dayjs(prevScheduleDate), 'day')
            // }
      ...
      },
      {
        ...
        cellRenderer: (params) => {
          if (params.data.id === 'total' || params.value === 'TOTAL') {
            return ''
            // return 'Totaltest' // 합계 행에서는 빈 문자열을 반환합니다.
          } else {
        ...
        valueSetter: params => {
          ...
          if (oldValue !== newValue) {
            ...
            // params.data.rIntCalcDays = diffDays
            ...
            // params.data.rSchdIntAmt = rSchdIntAmt
            // params.api.refreshCells({ rowNodes: [params.node], columns: ['rSchdIntAmt'], force: true })
            // params.api.refreshCells({ rowNodes: [params.node], columns: ['rIntCalcDays'], force: true })
            updateTotalRow()
            return true
          }
          return false
      ...
      {
        field: 'rIntCalcDays',
        headerName: i18n.global.t('label.view.rIntCalcDays'),
        width: 55,
        type: 'centerAligned',
        editable: false,
        cellRenderer: (params) => {
...
const updateTotalRow = () => {
  ...
  // console.log('Total round off:', totalRoundOff) // Add this line
...
//   console.log(gridProps.gridApi.value.scheduleParamValueCurrencyCodeValue)
//   const currency = currencyCodeList.value.find(c => c.code === gridProps.gridApi.value.scheduleParamValueCurrencyCodeValue)
//   if (currency.subUnitDigit !== undefined && currency.subUnitDigit >= 0 && value % 1 !== 0) {
//     value = value.toFixed(currency.subUnitDigit)
//   }
//   console.log(value)

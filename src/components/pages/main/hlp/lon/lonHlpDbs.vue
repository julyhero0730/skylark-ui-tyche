    ...
    <q-dialog allow-focus-outside v-model="model" persistent class="popup table_search_popup opened">
        <q-card class="popup_inner" :style=dragablePopupStyle>
            <q-card-section class="popup_header" v-touch-pan.mouse="onDragable">
                ...
                <div class="popup_title">{{ $t('label.view.lonHlpDbs') }}</div>
                <q-space />
                <q-btn flat v-close-popup><img src="~/assets/images/icon-frm-close.svg" alt="popup close button" ></q-btn>
            </q-card-section>
            <q-separator/>
            <q-card-section>
                <div class="modal_form table_search_modal height76">
                    <div class="search_condition_wrapper">
                        ...
                        <div style="margin-right: 10px;">
                          ...
                          <div>
                              ...
                              <CommonSelectBox class="width100" :codeList="commonCodeList.searchTypeList" v-model="searchParam.searchType.value" :disable="false"></CommonSelectBox>
                          </div>
                          <div>
...
const columnDefs = [
  ...
  {
    field: 'lastIntChrgPrd',
    hide: true
  },
  {
    field: 'nextSchdIntDate',
    hide: true
  },
  {
    field: 'accuralStopYn',
...
watch(() => searchParam.value.searchKeyword.value, (newVal) => {
  if (searchParam.value.searchType.value === 'DISBURSEMENT_NO' && newVal.length === 12) {
    search()
  }
})
